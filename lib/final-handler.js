...
function createRequestHandler(server, router, logger = console) {
  ...
  function buildMiddleware(context) {
    const { base } = context;
    const global_middlewares = server.global_middlewares || [];
    const base_middlewares = server.middlewares[base] || [];
    const routeHandlers = router.find(context.method, context.path) || {
      handlers: [],
    };
    context.req.params = routeHandlers.params || {};
    if (!routeHandlers.handlers.length && server.sub_applications[base]) {
  ...
  async function executeChain(middlewares, context) {
    ...
    const next = async (err) => {
      ...
      try {
        ...
        loggor.error("middlware failed ", err);
        throw err;
      }
    };
    return next();
